...
![Взаимодействие основных компонентов](https://raw.githubusercontent.com/ariverrun/coursework/main/assets/basic.architecture.png "Взаимодействие основных компонентов")
...
14. **Основные представления**
    ...
    3. [Многозадачность (concurrency).](https://github.com/ariverrun/coursework/blob/main/views/concurrency.md)
    4. [Инфраструктурное.](https://github.com/ariverrun/coursework/blob/main/views/infrastructure.md)
    5. [Безопасность.](https://github.com/ariverrun/coursework/blob/main/views/security.md)

15. **Анализ рисков созданной архитектуры, компромиссов**
    1. Риски в области безопасности.
        1. Как было указано в разделе посвященном инфраструктурном представлении, система делится на части находящиеся во внешней и внутренней подсети. И большая часть находится именно в подсети внутренней. Здесь защита от постороннего вмешательства во многом ложится на облачного провайдера. В целом маловероятно, что злоумышленник получит доступ в приватную подсеть, но можно сделать систему еще более защищенной, добавив систему авторизации для межсервисного взаимодействия. Но это окажет негативное влияние на производительность, а также потребуются дополнительные усилия и затраты на построение такой системы. Все те части, что находятся во внешней подсети (API Gateway) должны находиться под пристальным вниманием, следует очень тщательно подходить к их разработке и тестированию в том, что касается безопасности, ведь именно здесь будет находиться основной вектор атак. При этом эта часть является одной из самых нагруженных, так как через этот слой проходят все запросы пользователей. Необходимо будет искать баланс между производительностью и безопасностью, причем оба показателя крайне важны.
        2. Также существуют риски внутренних атак. Необходимо выстроить четкую систему уровней доступа, каждый сотрудник должен иметь прав доступа не больше, чем ему необходимо для решения задач. Выделение доступов должно быть наблюдаемым, все действия сотрудников также должны быть наблюдаемы.
    2. Риски, связанные с недостаточной способностью системы к масштабированию.
        1. Существует риск того, что система будет недостаточно быстро реагировать на повышение нагрузки, либо степень ее масштабирования будет недостаточной. Следует заранее проверять это методами нагрузочного тестирования. Если система не будет справляться с пользовательской активностью в тех регионах, где раньше ее не было (например, мы не работаем в стране X, а пользователи приезжают туда для проведения велопробега), то можно пересмореть подход к масштабированию Process Unit'ов и сделать его автоматическим.
        2. Перенос данных и редиректы запросов пользователя с уровня API Gateway при смене его локации являются потенциальным узким местом. Возможны ситуации, когда пользователь несколько раз в день переходит из зоны закрепленной за одним Process Unit'ом в закрепленную за другим. Проведение массовых соревнований или других спортивных мероприятий в таких условиях может стать проблемой. Можно рассмотреть подходы к закреплению пльзователей за конкретной зоной и направлять к ней при запросах из соседних зон или же реплицировать данные между соседними зонами (трейд-офф: дополнительный слой на уровне маршрутизации с снижением ее скорости или проблемы, порождаемые репликацией - конечная консистентность и др.)
    3. Риски связанные со сложностью системы и длительностью реализации.
        1. Мы разделили реализацию системы на несколько этапов с тем, чтобы максимально ускорить ее появление на рынке, но при этом с самого начала заложить основные подходы к построению архитектуры. При этом первый этап реализации по сути предполагает объединение уже имеющегося функционала и поэтому удастся использовать уже имеюищеся разработки и опыт, что ускорит выход на рынок.
        2. Ряд компонентов архитектуры сложны в реализации, при этом они не содержат логики уникальной для создаваемой системы, поэтому следует использовать для них готовые решения. Это относится к компонентам составляющим Virtualized Middleware и другим инфраструктурным элементам.

16. **Стоимость владения системой в первый, второй и пятый годы с учётом роста данных и базы пользователей**

    При рассчете стоимости владения мы опираемся на следующие оценки роста базы пользователей и других данных:

    ||Пользователи|Др. доменные сущности (заказы, товары, новости и т.д.)|Др. данные (логи и др.)|Итого|
    |---|----|----|----|----|
    |Первый год|700 тыс (10 ГБ)|6 ГБ|100 ГБ|116 ГБ|
    |Второй год|1.5 млн. (30 ГБ)|12 ГБ|400 ГБ|442 ГБ|
    |Пятый год|5 млн. (150 ГБ)|30 ГБ|800 ГБ|840 ГБ|

    Стоимость владения системой определяется следующими составляющими:
    1. Стоимость разработки
    2. Стоимость инфраструктуры
    3. Стоимость тестирования
    4. Стоимость поддержки

    Расходы на разработку и тестирование наиболее высоки в момент реализации архитектуры, далее они снижаются, но вырастает стоимость инфраструктуры и поддержки.

    За указанный период стоимость владения решением составит:
    1. Первый год: $ 3 млн.
    2. Второй год $ 5.5 млн.
    3. Пятый год: $ 8 млн.
